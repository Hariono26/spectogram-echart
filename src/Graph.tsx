...
const Graph: React.FC = () => {
  ...
  const asyncFetch = () => {
    ...
    fetch(url)
      ...
      .then((json) => {
        ...
        let yData = temp.map((a: { x: number }) => a.x);
        let zData = temp.map((a: { y: number }) => a.y);
        setDataFreq(yData);
        setDataMag(zData);
        setDataXY(temp)
      })
  ...
  const dataTemp = dataXY.map((item: {x: number, y:number}, index: number) => {
    return [index, 0, item.y]
  })

  // console.log(dataTemp)

  const data3Axis = dataTemp.map(function (item) {
    return [item[1], item[0], item[2] || "-"];
  });
